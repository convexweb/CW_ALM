...
namespace CW_ALM.WebAPI.Controllers
{
    ...
    public class UsuariosController : ControllerBase
    {
        ...
        public UsuariosController(
            IUsuarioRepository userService,
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromBody] UsuarioPostCreate request)
        {
        ...
        {
            var request = new UsuarioGetAllByStatus()
            {
                Status = status
            };
            var response = await _mediator.Send(request).ConfigureAwait(false);
        ...
        {
            var request = new UsuarioGetByUID()
            {
                UID = uID
            };
            var response = await _mediator.Send(request).ConfigureAwait(false);
        ...
        [Authorize]
        [HttpPut("UpdateAsync")]
        public async Task<IActionResult> UpdateAsync([FromBody] UsuarioPutEdit request)
        {
            var response = await _mediator.Send(request).ConfigureAwait(false);
            return Ok(response);
        }
        ...
        public async Task<IActionResult> InactivateAsync([FromBody] UsuarioPutInactivate request)
        {
            var response = await _mediator.Send(request).ConfigureAwait(false);
            return Ok(response);
        }
        ...
        public async Task<IActionResult> ReactivateAsync([FromBody] UsuarioPutReactivate request)
        {
            var response = await _mediator.Send(request).ConfigureAwait(false);
            return Ok(response);
        }
